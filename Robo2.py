...
# API key for OpenAI
openai_api_key = os.getenv("OPENAI_API_KEY_LAURA_TIER_2")
client = OpenAI(
...
def wait_for_batches_to_complete(batch_job_ids, batches_protocol, idx, failed_task):
    while True:
        ...
        if completed_batches + failed_batches == total_batches:
            ...
            if failed_batches > 0:
                batches_protocol.at[idx, failed_task] = failed_batch_ids
            return  # Exit the function
...
def insert_data_into_table(connection, cursor, table_name, dataframe, columns, batch_size=5000):
    try:
        insert_query = f"""
...
def upsert_data_into_table(connection, cursor, table_name, dataframe, columns, conflict_column, batch_size=5000):
    try:
        ...
        for i in range(0, len(rows_to_insert), batch_size):
            ...
            connection.commit()
            print(f"Upserted batch {i // batch_size + 1} with {len(batch)} rows")
...
# Prepare how the data is splittedl into batches
...
# Convert to object type so it can store lists
batches_protocol['failed_topic_job_ids'] = batches_protocol['failed_topic_job_ids'].astype(object)
batches_protocol['failed_sentiment_job_ids'] = batches_protocol['failed_sentiment_job_ids'].astype(object)



# Iterate over batches_protocol
for idx in range(0, len(batches_protocol), 2):  # Process two batches at a time
# for idx in range(0, min(2, len(batches_protocol)), 2):
    ...
    batches_protocol.to_csv("/home/ubuntu/test_laura/batches_protocol.csv") # save
    ...
    # Saving the collected sentences from 1 - topic recognition
    try:
    ...
    # Cleaning the ratings dataframe to only have integer values
    df_ratings['rating_food'] = df_ratings['rating_food'].apply(
    lambda x: np.nan if isinstance(x, list) or (isinstance(x, str) and x.startswith('[') and x.endswith(']')) else x
    )

    df_ratings['rating_service'] = df_ratings['rating_service'].apply(
        lambda x: np.nan if isinstance(x, list) or (isinstance(x, str) and x.startswith('[') and x.endswith(']')) else x
    )

    df_ratings['rating_atmosphere'] = df_ratings['rating_atmosphere'].apply(
        lambda x: np.nan if isinstance(x, list) or (isinstance(x, str) and x.startswith('[') and x.endswith(']')) else x
    )

    df_ratings["rating_food"] = df_ratings["rating_food"].fillna(-1).astype(float).apply(round).astype(int)
    df_ratings["rating_service"] = df_ratings["rating_service"].fillna(-1).astype(float).apply(round).astype(int)
    df_ratings["rating_atmosphere"] = df_ratings["rating_atmosphere"].fillna(-1).astype(float).apply(round).astype(int)
    ...
    try:
        ...
        upsert_data_into_table(connection, cursor, table, df_ratings, columns, conflict_column="review_id")

    except Exception as e:
        print(f"Failed to save to extracted topic sentences into the database{e}")


    ### Changing -1 values to None
    try:
        table_name = "reviews_subcategories"
        update_query = f"""
            UPDATE {table_name} 
            SET 
            rating_food = NULLIF(rating_food, -1),
            rating_service = NULLIF(rating_service, -1),
            rating_atmosphere = NULLIF(rating_atmosphere, -1)
            WHERE rating_food = -1 OR rating_service = -1 OR rating_atmosphere = -1;
            """
        cursor.execute(update_query)
        connection.commit()
        print("Updated na values")
    except Exception as e:
        print(f"Error updating the data into '{table_name}': {str(e)}")
        connection.rollback()
